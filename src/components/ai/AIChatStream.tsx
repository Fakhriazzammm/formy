...
const AIChatStream: React.FC<AIChatStreamProps> = ({ initialPrompt }) => {
  const [info, setInfo] = useState<string | null>(null);
  const setComponents = useFormStore.setState;
  
  // Gunakan hook useChat dari AI SDK dengan streaming
  ...
  // Efek untuk memproses respons streaming saat masih loading
  useEffect(() => {
    if (isLoading && messages.length > 0) {
      const lastMessage = messages[messages.length - 1];
      if (lastMessage.role === 'assistant') {
        // Coba parse komponen dari respons streaming yang sedang berlangsung
        const parsed = tryParseComponentsFromAI(lastMessage.content);
        if (parsed) {
          setComponents({ components: parsed });
          setInfo("Live preview form diupdate secara real-time!");
        }
      }
    }
  }, [messages, isLoading, setComponents]);
  ...
  return (
    <div style={{
      ...
      <h3>AI Chat Form Builder (Streaming)</h3>
